...
def list_user_files(user_id: str):
    """
    List all file metadata items for a user, enhancing each with
    information about available PDF previews.
    """
    try:
        # 1. Get all file records for the user from DynamoDB
        response = metadata_table.query(
            KeyConditionExpression=Key("user_id").eq(user_id)
        )
        items = response.get("Items", [])

        # 2. Enhance each item with preview information
        enhanced_items = []
        for item in items:
            original_s3_key = item.get("s3_key")
            # By default, assume no special preview key is available
            item['preview_s3_key'] = None

            if original_s3_key:
                # If the original file is already a PDF, it can be previewed directly
                if original_s3_key.lower().endswith('.pdf'):
                    item['preview_s3_key'] = original_s3_key
                else:
                    # If not a PDF, check if a converted version exists in S3
                    converted_key = f"{original_s3_key}.converted.pdf"
                    try:
                        s3_client.head_object(Bucket=S3_BUCKET_NAME, Key=converted_key)
                        # If it exists, set it as the preview key
                        item['preview_s3_key'] = converted_key
                        logger.info(f"Found preview for {original_s3_key} at {converted_key}")
                    except ClientError as e:
                        # If the converted file is not found (404), that's fine.
                        # The preview_s3_key will correctly remain None.
                        if e.response['Error']['Code'] != '404':
                            logger.warning(f"S3 error checking for {converted_key}: {e}")

            enhanced_items.append(item)

        # 3. Return the enhanced list of files
...
async def handle_upload(file: UploadFile, user: dict) -> dict:
    ...
    try:
        ...
        s3_key = f"uploads/{user['Username']}/{file.filename}"
        file.file.seek(0)
...
def generate_presigned_url(user_id: str, file_hash: str, expires_in: int = 3600) -> str:
    """
    Generate a (temporary) presigned download URL for a user's file.
    If a converted PDF version exists for a non-PDF file, it returns the URL for the PDF.
    """
    if not S3_BUCKET_NAME:
        raise RuntimeError("S3_BUCKET_NAME not set in environment.")

    try:
        # Step 1: Get the file's metadata from DynamoDB
        result = metadata_table.get_item(Key={"user_id": user_id, "hash": file_hash})
        if "Item" not in result or not result["Item"]:
            raise HTTPException(status_code=404, detail="File metadata not found.")
        ...
        # --- Start of New Logic ---
        key_to_use = original_s3_key  # Default to the original key

        # Check if the original file is not a PDF
        if not original_s3_key.lower().endswith('.pdf'):
            # If not a PDF, check if a converted version exists in S3
            converted_key = f"{original_s3_key}.converted.pdf"
            try:
                # Use head_object to efficiently check for the file's existence
                s3_client.head_object(Bucket=S3_BUCKET_NAME, Key=converted_key)
                # If it exists, update the key_to_use for the URL generation
                key_to_use = converted_key
                logger.info(f"Found converted PDF. Using key: {key_to_use}")
            except ClientError as e:
                # A 404 error means the converted file wasn't found, which is okay.
                # We'll just use the original key. For any other error, we should stop.
                if e.response['Error']['Code'] != '404':
                    raise e  # Re-raise unexpected S3 errors
        # --- End of New Logic ---

        # Generate the presigned URL using the determined key (either original or converted)
        url = s3_client.generate_presigned_url(
            'get_object',
            Params={'Bucket': S3_BUCKET_NAME, 'Key': key_to_use},
            ExpiresIn=expires_in
        )
        return url

    except ClientError as e:
        logger.error(f"DynamoDB or S3 ClientError: {e}")
        raise HTTPException(status_code=500, detail="Error accessing AWS for presigned URL.")
    except Exception as e:
        logger.error(f"Unexpected error during presigned URL generation: {e}")
        raise HTTPException(status_code=500, detail="Could not generate presigned URL.")
