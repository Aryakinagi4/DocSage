...
from file_handler import delete_all_user_files_and_metadata
from utils import delete_user_from_cognito
...
async def delete_user_account(authorization: str = Header(None)):
    ...
    This includes files in S3, metadata in DynamoDB, and the user in Cognito.
    """
    if not authorization:
        raise HTTPException(status_code=401, detail="Authorization header missing.")
    ...
    try:
        ...
        # Step 1: Delete all files from S3 and metadata/conversations from DynamoDB
        logger.info(f"Initiating data deletion for user: {username}")
        delete_all_user_files_and_metadata(username)
        
        # Step 2: Delete the user from the Cognito user pool
        logger.info(f"Initiating Cognito account deletion for user: {username}")
        delete_user_from_cognito(username)
        
        return {"message": "User account and all associated data have been successfully deleted."}
        
    except HTTPException as e:
        # Re-raise known HTTP exceptions
        raise e
    except Exception as e:
        logger.exception(f"An unexpected error occurred during account deletion: {e}")
