...
export default function Dashboard() {
      ...
      try {
        ...
        if (listData.files) {
          const formattedConversations = listData.files.map(file => ({
            ...
            fileHash: file.hash,
            messages: [],
            previewS3Key: file.preview_s3_key
          }));
  ...
  const handleConversationClick = async (conversation) => {
    ...
    try {
      // Download file and get URL
      const fileUrl = await downloadFile(conversation, token);
      console.log("DEBUG: URL being sent to the PDF viewer:", fileUrl);
      setPdfUrl(fileUrl);
              ...
              <PanelGroup direction="horizontal" className="chat-container">
                <Panel defaultSize={50} minSize={30} className="pdf-viewer-panel">
                  {pdfUrl ? (
                    <DocumentViewer
                      fileUrl={pdfUrl}
                      fileName={activeConversation?.previewS3Key ? activeConversation.name.replace(/\.[^/.]+$/, ".pdf") : fileName}
                      onError={(error) => {
                  ...
                  {/* {activeConversation ? (
                    // If the active document has a preview key, try to load it
                    activeConversation.previewS3Key ? (
                      pdfUrl ? (
                        <DocumentViewer
                          fileUrl={pdfUrl}
                          fileName={activeConversation?.previewS3Key ? activeConversation.name.replace(/\.[^/.]+$/, ".pdf") : fileName}
                      ...
                      // If there is NO preview key, show the "cannot be previewed" message
                      <div className="no-preview-container">
                        <div className="no-preview-icon">📝</div>
                        <h3>{fileName.split('.').pop().toUpperCase()} Document</h3>
                        <p>This file type cannot be previewed in the browser.</p>
                        <p className="note">Note: You can still ask questions about this document using the chat interface.</p>
                      </div>
                    )
                  ) : (
                    // If no document is selected at all, show the initial upload box
                    <div className="upload-box">
                      <label htmlFor="fileUpload" className="upload-label">Upload a document</label>
                      <input type="file" id="fileUpload" accept=".pdf,.docx,.doc,.pptx,.ppt,.xlsx,.xls,.txt" onChange={handleFileUpload} />
                    </div>
                  )} */}
                </Panel>
                <PanelResizeHandle className="resize-handle" />
